...
def CNN_model_first(input_shape, num_classes):
...
def CNN_model_second(input_shape, num_classes):
...
def trainAndPredictCNN(X_train, Y_train, X_test, Y_test):
    ...
    model_first = CNN_model_first((28,28,1), num_classes)
    model_second = CNN_model_second((28,28,1), num_classes)

    #TODO - Application 2 - Step 8 - Train the model
    history_first = model_first.fit(X_train, Y_train, validation_data=(X_test, Y_test),
              epochs=5, batch_size=200)
    history_second = model_second.fit(X_train, Y_train, validation_data=(X_test, Y_test),
                    epochs=5, batch_size=200)

    #TODO - Application 2 - Step 8 - System evaluation - compute and display the prediction error
    # scores_first = model_first.evaluate(X_test,Y_test, verbose=0)
    # scores_second = model_first.evaluate(X_test,Y_test, verbose=0)

    # the error will be different because the initialization is random
    # it depends on the subset of training data being selected
    # print("CNN Error: {:.2f}".format(100 - scores_first[1] * 100))
    line0, = plt.plot(history_first.epoch, history_first.history['accuracy'])
    line1, = plt.plot(history_second.epoch, history_second.history['accuracy'])
    ...
    print(history_first.history['accuracy'])
    print(history_second.history['accuracy'])
