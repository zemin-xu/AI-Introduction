...
#####################################################################################################################
#####################################################################################################################
def defineModel(input_shape, num_classes, learning_rate):
    ...
    model.compile(SGD(lr=learning_rate, momentum=0.9) ,loss='categorical_crossentropy',
                  metrics=['accuracy'])
...
def trainAndEvaluateClassic(trainX, trainY, testX, testY):
    ...
    #TODO - Application 1 - Step 7 - Train the model
        model = defineModel((28, 28, 1), 10, learning_rates[i])
        history = model.fit(trainX, trainY, callbacks=[time_callback], epochs = 5, batch_size=32, validation_data=(testX, testY), verbose=1)
    ...
    plt.legend([line0, line1, line2, line3, line4],
               [learning_rates[0], learning_rates[1], learning_rates[2], learning_rates[3], learning_rates[4]])
    plt.xlabel('epochs')
    plt.ylabel('accuracy')
    ...
    for i in range(len(times)):
        ...
        print("the average time for each epoch in case: ",learning_rates[i], " of neuron is: ", mean)
