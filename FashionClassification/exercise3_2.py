...
#####################################################################################################################
#####################################################################################################################
def defineModel(input_shape, num_classes, num_filter):
...
def trainAndEvaluateClassic(trainX, trainY, testX, testY):
    ...
    nums_filter = [8, 16, 32, 64, 128]
    histories =[]

    #Application 1 - Call the defineModel function

    for i in range(len(nums_filter)):
    #TODO - Application 1 - Step 7 - Train the model
        model = defineModel((28, 28, 1), 10, nums_filter[i])
    ...
    for j in range(len(nums_filter)):
    ...
    plt.legend([line0, line1, line2, line3, line4 ],
               [nums_filter[0], nums_filter[1], nums_filter[2], nums_filter[3], nums_filter[4]])
    plt.xlabel('epochs')
    plt.ylabel('accuracy')
    ...
    for i in range(len(times)):
        ...
        print("the average time for each epoch in case: ",nums_filter[i], " of neuron is: ", mean)
