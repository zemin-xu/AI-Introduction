...
def trainAndEvaluateClassic(trainX, trainY, testX, testY):
    ...
    nums_epoch = [1, 2, 5, 10, 20]
    histories =[]

    #Application 1 - Call the defineModel function

    for i in range(len(nums_epoch)):
    #TODO - Application 1 - Step 7 - Train the model
        model = defineModel((28, 28, 1), 10)
        history = model.fit(trainX, trainY, callbacks=[time_callback], epochs = nums_epoch[i], batch_size=32, validation_data=(testX, testY), verbose=1)
    ...
    plt.legend([line0, line1, line2, line3, line4],
               [nums_epoch[0], nums_epoch[1], nums_epoch[2], nums_epoch[3], nums_epoch[4]])
    plt.xlabel('epochs')
    plt.ylabel('accuracy')
    ...
    for i in range(len(times)):
        ...
        print("the average time for each epoch in case: ",nums_epoch[i], " of neuron is: ", mean)
