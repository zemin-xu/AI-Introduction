...
#####################################################################################################################
#####################################################################################################################
def defineModel(input_shape, num_classes, has_padding):
...
def trainAndEvaluateClassic(trainX, trainY, testX, testY):
    ...
    model_padding = defineModel((28, 28, 1), 10, True)
    history_padding = model_padding.fit(trainX, trainY, callbacks=[time_callback], epochs = 4, batch_size=32, validation_data=(testX, testY), verbose=1)

    model_no= defineModel((28, 28, 1), 10, False)
    history_no= model_no.fit(trainX, trainY, callbacks=[time_callback], epochs = 4, batch_size=32, validation_data=(testX, testY), verbose=1)
    ...
    line0, = plt.plot(history_padding.epoch, history_padding.history['accuracy'])
    line1, = plt.plot(history_padding.epoch, history_padding.history['val_accuracy'])
    line2, = plt.plot(history_no.epoch, history_no.history['accuracy'])
    line3, = plt.plot(history_no.epoch, history_no.history['val_accuracy'])
    ...
    plt.legend([line0, line1, line2, line3],
               ["accuracy_padding", "val_accuracy_padding", "accu_no_padding","val_accu_no_padding"])
    plt.xlabel('epochs')
    plt.ylabel('accuracy')
